...
export interface PaddedCanvas {
  image: RgbaImage;
  /** Subtract these offsets from canvas coordinates to map back to src. */
  offsetX: number;
  offsetY: number;
}

/**
 * Center src on a black canvas `factor` times larger ("zoom out"). SCRFD is
 * trained on scenes where faces are small relative to the frame; tight head
 * crops score poorly unless padded out first.
 */
export function padToCanvas(src: RgbaImage, factor: number): PaddedCanvas {
  const width = Math.round(src.width * factor);
  const height = Math.round(src.height * factor);
  const offsetX = Math.floor((width - src.width) / 2);
  const offsetY = Math.floor((height - src.height) / 2);
  ...
  return { image: { width, height, data }, offsetX, offsetY };
}
